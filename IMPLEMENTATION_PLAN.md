...
### 已確認決策
- **每日儲存確認 Modal (`saveModal`)**：整個移除，存檔恢復安靜完成，不跳窗。
- **週日結算 Modal (`sundayModal`)**：保留但簡化。移除詳細比較表格（改由常駐面板呈現），只留簡短確認訊息（起點已建立 / 本週三圍已記錄）。
- **新增常駐「週進度」面板**：位置在三張 Bento 卡片下方、長期趨勢圖表上方。內容分兩塊：
  - 🏁 **總計**：起點週日 → 最新週日的變化
  - 📅 **小計**：逐週清單，顯示全部歷史週次（起點到最新全部列出），清單加內部捲軸
- 資料來源改為每次 `refreshUI()` 時從全部紀錄重新計算週日資料，不再依賴「剛存檔」的事件，任何時間打開網頁都看得到最新完整進度。

### 影響檔案
1. `src/main.js`：移除 `showSaveModal()` 及其呼叫/監聽；簡化 `showSundayModal()`；`refreshUI()` 新增呼叫 `renderWeeklyProgress(records)`
2. `src/dashboard.js`：新增 `renderWeeklyProgress(records)` 計算總計與逐週小計
3. `index.html`：移除 `saveModal` 區塊；簡化 `sundayModal` 內部結構；新增「週進度」面板 HTML 骨架

預估變更 <150 行，單一 `task.md` checkbox 完成。
